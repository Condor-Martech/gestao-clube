...
console.log(
  '  Key:',
  usingServiceRole ? 'service_role (bypasses RLS)' : 'publishable/anon (subject to RLS)',
)
...
section('campanhas — process breakdown')
{
  ...
  print({
    ...
    errors: {
      total: t.error?.message,
      processTrue: tt.error?.message,
      processFalse: tf.error?.message,
    },
  })
}
...
section('Produtos — column population analysis (sync candidates)')
{
  ...
  const aproved = await supabase
    .from(TABLE)
    .select('id', { count: 'exact', head: true })
    .eq('aproved', true)
  const imgInternal = await supabase
    .from(TABLE)
    .select('id', { count: 'exact', head: true })
    .not('img_internal', 'is', null)
  ...
  const aprovedAndImg = await supabase
    .from(TABLE)
    .select('id', { count: 'exact', head: true })
    .eq('aproved', true)
    .not('img_internal', 'is', null)
  const aprovedAndHost = await supabase
    .from(TABLE)
    .select('id', { count: 'exact', head: true })
    .eq('aproved', true)
    .not('host', 'is', null)
  const aprovedAndPrice = await supabase
    .from(TABLE)
    .select('id', { count: 'exact', head: true })
    .eq('aproved', true)
    .not('price', 'is', null)
...
section('Produtos — distinct campanhas by sync criteria')
{
  ...
  print({
    ...
    'aproved AND img_internal NOT NULL': await fetchDistinctCampanhas((q) =>
      q.eq('aproved', true).not('img_internal', 'is', null),
    ),
    'aproved AND host NOT NULL': await fetchDistinctCampanhas((q) =>
      q.eq('aproved', true).not('host', 'is', null),
    ),
    'aproved AND price NOT NULL': await fetchDistinctCampanhas((q) =>
      q.eq('aproved', true).not('price', 'is', null),
    ),
    'img_internal NOT NULL (sync only, no aproved)': await fetchDistinctCampanhas((q) =>
      q.not('img_internal', 'is', null),
    ),
  })
}
